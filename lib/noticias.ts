...
export const CACHE_KEY = "noticias:processed";
export const STALE_KEY = "noticias:processed:stale";
export const TTL = 60 * 60;           // 1 h — datos frescos
export const STALE_TTL = 6 * 60 * 60; // 6 h — fallback cuando fresh expira
...
export function getMockNoticias(): Noticia[] {
  return [
    {
      id: "1",
...
export async function generarNoticias(): Promise<Noticia[]> {
...
${articlesText}`,
  ...
  return parsed.map((n, i) => {
    const original = articles.find((a) =>
      a.titulo.toLowerCase().includes(String(n.titulo).toLowerCase().slice(0, 20))
    ) ?? articles[i];
